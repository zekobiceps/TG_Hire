...
with tab1:
    ...
    with col1:
        ...
        competences_annonce = st.text_area("Compétences clés requises:", placeholder="Ex: React, Node.js, MongoDB, AWS", height=100)
    
    with col2:
        # Option pour importer depuis le brief
        if st.session_state.poste_intitule:
            if st.button("📋 Importer depuis le brief"):
                poste_annonce = st.session_state.poste_intitule
                # Essayer d'extraire les compétences du brief
                competences_text = ""
