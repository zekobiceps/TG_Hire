...
# Afficher la saisie manuelle uniquement pour TGCC
if plateforme == "TGCC":
    input_mode = st.radio("Mode d'entrée des infos du poste", ["Upload PDF fiche de poste", "Saisie manuelle"], key="input_mode_2")
else:
    input_mode = "Upload PDF fiche de poste"
...
if input_mode == "Upload PDF fiche de poste":
    uploaded_file = st.file_uploader("Téléchargez la fiche de poste (PDF)", type="pdf", key="pdf_upload_2")
    if uploaded_file:
        try:
            pdf_reader = pypdf.PdfReader(BytesIO(uploaded_file.read()))
            fiche_text = ""
    ...
    with col1:
        type_contrat = st.selectbox("Type de contrat*", 
            ["CDI", "CDD", "Contrat de chantier", "Stage"],
            help="Sélectionnez le type de contrat", key="type_contrat_2")
        niveau_experience = st.selectbox("Niveau d'expérience*",
            ["0-2 ans", "3-5 ans", "5-10 ans", "10+ ans"],
            help="Nombre d'années d'expérience requis", key="niveau_experience_2")
        formation_requise = st.selectbox("Formation requise*",
            ["Bac", "Bac+2", "Bac+3", "Bac+5"],
    ...
    with col2:
        ...
        date_demarrage = st.text_input("Date de démarrage", help="Ex: Dès que possible, Septembre 2024", key="date_demarrage_2")
        salaire = st.text_input("Salaire indicatif", help="Ex: Selon profil et expérience", key="salaire_2")
        
    with col3:
        competences_techniques = st.text_area("Compétences techniques*", 
            help="Ex: Maîtrise d'AutoCAD, gestion de projet, normes BTP, lecture de plans", key="competences_techniques_2")
        langues = st.multiselect("Langues requises",
            ["Arabe", "Français", "Anglais", "Espagnol"],
            help="Langues nécessaires pour le poste", key="langues_2")
        
    with col4:
        missions_principales = st.text_area("Missions principales*", 
            help="Ex: Gestion de chantier, coordination d'équipe, suivi budget, contrôle qualité", key="missions_principales_2")
        soft_skills = st.text_area("Soft skills*",
            help="Ex: Leadership, rigueur, autonomie, gestion du stress", key="soft_skills_2")
        deplacement = st.selectbox("Déplacements requis",
            ["Non", "Occasionnels", "Fréquents", "Permanent"],
            help="Fréquence des déplacements", key="deplacement_2")
    ...
    with col_ctx1:
        contexte_recrutement = st.text_area("Contexte du recrutement*",
            help="Ex: Expansion de l'entreprise sur de nouveaux marchés, nouveau chantier", key="contexte_recrutement_2")
        description_equipe = st.text_area("Description de l'équipe",
            help="Ex: Équipe de 15 ingénieurs expérimentés", key="description_equipe_2")
    ...
    with col_ctx2:
        ...
        avantages = st.text_area("Avantages proposés",
            help="Ex: Mutuelle, transport, formation, évolution de carrière", key="avantages_2")
...
col_save1, col_save2, col_save3 = st.columns([1, 2, 1])
with col_save2:
    if st.button("💾 Sauvegarder l'annonce", type="primary", width="stretch", key="btn_sauvegarder_annonce_2"):
        if poste_final and entreprise and contenu and localisation_finale:
            # Préparer les données pour Google Sheets
            annonce_data = {
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
