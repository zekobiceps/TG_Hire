...
# ---------------- NOUVELLES FONCTIONS POUR LA BIBLIOTHEQUE ----------------
def load_job_descriptions():
    """Charge les fiches de poste sauvegardées depuis job_descriptions.json"""
    try:
...
# ---------------- AUTRES FONCTIONS EXISTANTES (NON MODIFIEES) ----------------
...
# Initialisation pour la bibliothèque de fiches
...
with tabs[1]:
    ...
    # Bouton pour l'assistance IA (MODIFIE POUR UTILISER LA BIBLIOTHEQUE)
    if st.button("💡 Prérédiger avec l'IA", type="secondary", use_container_width=True, key="ai_fill_button"):
        with st.spinner("L'IA génère les suggestions..."):
            ...
            if job_title and job_title in st.session_state.saved_job_descriptions:
                # Utiliser la fiche de la bibliothèque
                extracted_data = st.session_state.saved_job_descriptions[job_title]
                for key, value in extracted_data.items():
                    if key in st.session_state:
                        st.session_state[key] = value
                st.success(f"✅ Suggestions chargées depuis la bibliothèque pour '{job_title}'.")
            else:
                # Fallback sur l'ancien comportement si pas trouvé dans la biblio
                ...
                st.info("ℹ️ Aucune fiche trouvée dans la bibliothèque. Suggestions génériques ajoutées.")
            st.rerun()
...
with tabs[4]:
    ...
    # Section pour uploader et analyser une nouvelle fiche
    ...
    if "temp_extracted_data" in st.session_state:
        ...
        # Bouton pour sauvegarder la fiche éditée
        if st.button("💾 Sauvegarder la fiche", type="primary", use_container_width=True):
            # Reconstruire le dict à partir de l'edited_df
            saved_data = {}
            for i in range(len(edited_df)):
            ...
            st.success(f"✅ Fiche pour '{job_key}' sauvegardée dans la bibliothèque.")
            del st.session_state.temp_extracted_data  # Nettoyer temp
            del st.session_state.temp_job_title
            st.rerun()

    # Afficher la liste des fiches sauvegardées
    ...
    if st.session_state.saved_job_descriptions:
        for job_title, data in st.session_state.saved_job_descriptions.items():
            with st.expander(f"📄 {job_title.capitalize()}"):
                ...
                edit_data = []
                for key, value in data.items():
                    edit_data.append([key.capitalize().replace("_", " "), value])
