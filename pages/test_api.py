...
# --- Gestion des appels API Google Sheets ---
import time

CACHE_FILE = "google_sheets_cache.json"
CACHE_EXPIRY = timedelta(minutes=5)  # Cache expiration time
...
def fetch_google_sheet_data():
    """Fetch data from Google Sheets with caching and rate limiting."""
    # Check cache first
...
def fetch_data_with_rate_limit():
    """Fetch data with a delay to avoid exceeding API rate limits."""
    for _ in range(10):  # Example: 10 API calls
        try:
            st.info("Appel à l'API Google Sheets avec délai...")
            time.sleep(1)  # Delay of 1 second between calls
            # Simulate API call (replace with actual API logic)
            data = {"example": "data"}  # Replace with actual data fetching logic
            st.write(data)
        except Exception as e:
