...
with tab7:
    ...
    def _perm_apply_format_callback():
        ...
        # Support domain-only formats like '@domaine.ma' or 'exemple@domaine.ma'
        domain_cb = None
        if email_format_cb:
            if email_format_cb.startswith('@'):
                domain_cb = email_format_cb.lstrip('@')
            elif '@' in email_format_cb:
        ...
        if source_cb == "Charika.ma" and email_format_cb and domain_cb:
            ...
            if ('prenom' in ef.lower() or 'prénom' in ef.lower() or 'nom' in ef.lower() or
                ...
                # If the user gave only a domain like '@domaine.ma', or a full example email
                # and it doesn't contain placeholders, try to detect if it's domain-only
                if email_format_cb.startswith('@') or ('@' in email_format_cb and email_format_cb.split('@',1)[0] == ''):
                    # domain-only -> generate standard permutations using this domain
                    patterns_cb = [
