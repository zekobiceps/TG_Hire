...
# Keywords for scoring (same as before)
keywords = {
...
print(f"Vérification de {len(user_files)} fichiers...\n")
...
for f in user_files:
    ...
    if not row.empty:
        ...
        r = row.iloc[0]
        # Check criteria
        exp = r['years_experience_clean']
        score = r['score']
        
        status = "✅ MATCH CRITÈRES" if (4 <= exp <= 12 and score > 0) else "❌ HORS CRITÈRES"
        reason = []
        if not (4 <= exp <= 12):
            reason.append(f"Exp {exp} ans (Attendu: 4-12)")
        if score == 0:
            reason.append("Score 0 (Pas de mots-clés RAF/Finance)")
            
        reason_str = ", ".join(reason) if reason else "OK"
        
        print(f"Dossier: {f} -> Trouvé. {status}. [{reason_str}]")
        print(f"   Nom: {r['candidate_name']}, Cat: {r['sub_category']}, Score: {score}")
    else:
        missing_files.append(f)
        print(f"Dossier: {f} -> ⚠️ PAS DANS LE FICHIER DE RÉSULTATS")

print("-" * 50)
print(f"Total fournis: {len(user_files)}")
print(f"Trouvés dans la base: {found_count}")
print(f"Manquants: {len(missing_files)}")

if found_count > 0:
    print("\nCeux qui ont matché (Exemples):")
    matches = [f for f in user_files if not df[(df['file'] == f) & (df['years_experience_clean'] >= 4) & (df['years_experience_clean'] <= 12) & (df['score'] > 0)].empty]
    print(f"Nombre de matchs parmi cette liste : {len(matches)}")
