...
# Paths
INPUT_CSV = 'LOGO/CVS/classification_results.csv'
CVS_DIR = 'LOGO/CVS/'
OUTPUT_ZIP = 'Candidats_RAF_Score_Sup_5.zip'

# Specific files list provided by user (Same as before)
user_files = [
...
# Filter: In user_list AND exp 4-12 AND score > 5
...
print(f"Found {len(selected_candidates)} candidates with score > 5.")

# Create ZIP
with zipfile.ZipFile(OUTPUT_ZIP, 'w') as zipf:
    for _, row in selected_candidates.iterrows():
        filename = row['file']
        source_path = os.path.join(CVS_DIR, filename)
        
        # Use candidate name + score in filename for better readability in ZIP
        clean_name = str(row['candidate_name']).replace(' ', '_').replace('/', '-')
        new_filename = f"{clean_name}_Score{row['score']}_{filename}"
        
        if os.path.exists(source_path):
            print(f"Adding {filename} as {new_filename}")
            zipf.write(source_path, arcname=new_filename)
        else:
            print(f"Warning: File {filename} not found at {source_path}")
